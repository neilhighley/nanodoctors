...
half3 albedo = FluvioAlbedo(i_tex, float3(1, 1, 1));
#endif
half2 metallicGloss = MetallicGloss(i_tex.xy);half metallic = metallicGloss.x;half oneMinusRoughness = metallicGloss.y;half oneMinusReflectivity;half3 specColor;half3 diffColor = DiffuseAndSpecularFromMetallic(albedo, metallic,  specColor,  oneMinusReflectivity);
#ifdef _VERTEXCOLORMODE_SPECULAR
...
#endif
inline FragmentCommonData FluvioFragmentSetup (float2 i_tex, half3 i_eyeVec, half3 i_normalWorld, half3x3 i_tanToWorld, half3 i_posWorld, half3 vertex_color){half alpha = FluvioAlpha(i_tex);clip(_CullFluid);
#ifdef _NORMALMAP
...
#endif
o.pos = mul(UNITY_MATRIX_MVP, v.vertex);o.tex = FluvioTexCoords(v);o.eyeVec = NormalizePerVertexNormal(posWorld.xyz - _WorldSpaceCameraPos);float3 normalWorld = FluvioGetWorldNormal(v.normal);
#ifdef _TANGENT_TO_WORLD
